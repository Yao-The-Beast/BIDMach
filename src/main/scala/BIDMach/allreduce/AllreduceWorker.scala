...
//   println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Actor id = ${workerId}")
//   println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Thresholds: thReduce = ${thReduce}, thComplete = ${thComplete}");

//   def receive = {

//     case p: PrepareAllreduce => {

//       log.info(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Recieve prepare data round ${p.round}")
...
//           log.warning(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Force completing round ${p.round}")
...
//           maxChunkSize = maxChunkSize
//         )
//         print(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Sending confirm to master: $master....")
...
//         println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Prepare round ${p.round}")
//         println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Size of scatter buffer: ${scatterBlockBuf.maxLag} x ${scatterBlockBuf.peerSize} x ${scatterBlockBuf.dataSize}. threshold : ${scatterBlockBuf.minChunkRequired}")
//         println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Size of reduce buffer: ${reduceBlockBuf.maxLag} x ${reduceBlockBuf.peerSize} x ${reduceBlockBuf.maxBlockSize}. threshold: ${reduceBlockBuf.minChunkRequired}")
...
//             val path = peerAddress.path / s"DimensionNode?dim=${dim}"/s"Worker"
//             println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Path to query ref: $path")
...
//     case s: ScatterBlock => {
//       try {
//         log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Receive scattered data from round ${s.round} srcId = ${s.srcId}, destId = ${s.destId}, chunkId=${s.chunkId}")
//         handleScatterBlock(s);
//       } catch {
//         case e: Throwable => printStackTrace("scatter block", e);
//       }
...
//     case r: ReduceBlock => {
//       try {
//         log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Receive reduced data from round ${r.round}, srcId = ${r.srcId}, destId = ${r.destId}, chunkId=${r.chunkId}")
//         handleReduceBlock(r);
//       } catch {
//         case e: Throwable => printStackTrace("reduce block", e);
//       }
...
//   private def handleReduceBlock(r: ReduceBlock) = {
//     if (r.value.size > maxChunkSize) {
//       throw new RuntimeException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Reduced block of size ${r.value.size} is larger than expected.. Max msg size is $maxChunkSize")
//     } else if (r.destId != workerId) {
//       throw new RuntimeException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Message with destination ${r.destId} was incorrectly routed to node $workerId")
//     } else if (r.round > currentRound) {
//       throw new RuntimeException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: New round ${r.round} should have been prepared, but current round is $currentRound")
//     }

//     if (r.round < currentRound) {
//       log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Outdated reduced data")
//     } else {
//       reduceBlockBuf.store(r.value, r.round, r.srcId, r.chunkId, r.count)
//       if (reduceBlockBuf.reachCompletionThreshold(r.round)) {
//         log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Receive enough reduced data (numPeers = ${peers.size}) for round ${r.round}, complete")
//         completeRound()
//       }
//     }
//   }
...
//     if (s.destId != workerId) {
//       throw new RuntimeException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Scatter block should be directed to $workerId, but received ${s.destId}")
//     } else if (s.round > currentRound) {
//       throw new RuntimeException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: New round ${s.round} should have been prepared, but current round is $currentRound")
//     }

//     if (s.round < currentRound) {
//       log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Outdated scattered data")
//     } else {
//       scatterBlockBuf.store(s.value, s.round, s.srcId, s.chunkId)
//       if (scatterBlockBuf.reachReducingThreshold(s.round, s.chunkId)) {
//         log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: receive ${scatterBlockBuf.count(s.round, s.chunkId)} scattered data (numPeers = ${peers.size}), chunkId =${s.chunkId} for round ${s.round}, start reducing")
//         reduceAndBroadcast(s.chunkId)
//       }
//     }
...
//     val input = dataSource(AllReduceInputRequest(currentRound))
//     if (dataSize != input.data.size) {
//       throw new IllegalArgumentException(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Input data size ${input.data.size} is different from initialization time $dataSize!")
//     }
//     data = input.data
//   }
...
//   private def flush() = {
//     reduceBlockBuf.getWithCounts(currentRound, output, outputCount)
//     log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Flushing output at completed round $currentRound")
//     dataSink(AllReduceOutput(output, outputCount, currentRound))
//   }

//   private def scatter() = {
//     for (peerId <- 0 until peerNum) {
//       val idx = (peerId + workerId) % peerNum
...
//         System.arraycopy(data, blockStart + chunkStart, chunk, 0, chunkSize);
//         log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Send msg from ${workerId} to ${idx}, chunkId: ${i}")
//         val scatterMsg = ScatterBlock(chunk, workerId, idx, i, currentRound)
//         if (worker == self) {
//           handleScatterBlock(scatterMsg)
//         } else {
...
//   private def broadcast(data: Array[Float], chunkId: Int, reduceCount: Int) = {
//     log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Start broadcasting")
//     for (i <- 0 until peerNum) {
//       val peerworkerId = (i + workerId) % peerNum
//       val worker = peers(peerworkerId)
//       log.debug(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Broadcast reduced block src: ${workerId}, dest: ${peerworkerId}, chunkId: ${chunkId}, round: ${currentRound}")
//       val reduceMsg = ReduceBlock(data, workerId, peerworkerId, chunkId, currentRound, reduceCount)
//       if (worker == self) {
//         handleReduceBlock(reduceMsg)
//       } else {
...
//       if (r.iteration % checkpoint == 0) {
//         val inputUsed = randomFloats(r.iteration % totalInputSample)
//         // println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Asserting #${r.iteration} output...")
//         var zeroCountNum = 0
//         var totalCount = 0
//         for (i <- 0 until sourceDataSize) {
//           val count = r.count(i)
...
//         // println(s"\n----Worker ${self.path}: Worker workerId={$workerId} dim={$dim}: Data output at #${r.iteration} - $timeElapsed s")
//         val bytes = r.data.length * 4.0 * checkpoint
//         val mBytes = bytes / 1.0e6
//         val throughput = mBytes / timeElapsed
